...
int	rwnx_plat_userconfig_load_8800d80(struct rwnx_hw *rwnx_hw){
    ...
    char *filename = FW_USERCONFIG_NAME_8800D80;
...
#ifndef ANDROID_PLATFORM
            {
                ...
                int chip_dir_len = (int)(sizeof("aic8800D80") - 1);

                /* aic_fw_path may already point into the aic8800D80 dir,
                 * e.g. inherited from aic_load_fw via get_fw_path() when the
                 * loader was loaded with aic_fw_path=/lib/firmware/aic8800D80.
                ...
                if (path_len < chip_dir_len ||
                    strncmp(aic_fw_path + path_len - chip_dir_len, "aic8800D80", chip_dir_len)) {
                    snprintf(temp_path, sizeof(temp_path), "%s/%s", aic_fw_path, "aic8800D80");
